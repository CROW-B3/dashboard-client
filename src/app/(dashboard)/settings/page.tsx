...
export default function SettingsPage() {
  ...
  const { data: billing, isLoading: billingLoading } = useQuery<{ plan?: string; status?: string; currentPeriodEnd?: string; billingPeriod?: string } | null>({
    queryKey: ['billing', orgUuid],
    queryFn: async () => {
      const res = await fetch(`${API_GATEWAY_URL}/api/v1/billing/subscriptions/${orgUuid}`, { credentials: 'include' });
      ...
      {activeTab === 'profile' && (
        <GlassPanel>
          ...
          <div className="space-y-4">
            ...
            <button onClick={handleSaveProfile} disabled={saveProfileMutation.isPending} className="px-4 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-lg text-sm transition-colors disabled:opacity-50">
        ...
        <GlassPanel>
          ...
          <div className="space-y-4">
            <div className="flex gap-2">
              ...
              />
              <Button disabled={!keyName.trim() || createKeyMutation.isPending} onClick={handleCreateKey}>
            ...
            <div className="space-y-2">
              {apiKeys?.map((k: ApiKeyRecord) => (
                <div key={k.id} className="flex items-center justify-between p-3 bg-white/[0.03] border border-white/10 rounded-lg">
                  <div>
                    <p className="text-sm text-white">{k.name || 'Unnamed key'}</p>
          ...
          {billingLoading ? (
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-sm border-b border-white/10 py-2">
                ...
                <span className="text-white">{billing.billingPeriod || 'Free'}</span>
              </div>
              <div className="flex justify-between text-sm border-b border-white/10 py-2">
                <span className="text-gray-400">Status</span>
                <span className={billing.status === 'active' ? 'text-green-400' : 'text-gray-300'}>{billing.status || 'Unknown'}</span>
              </div>
              {billing.currentPeriodEnd && (
                <div className="flex justify-between text-sm border-b border-white/10 py-2">
                  <span className="text-gray-400">Renews</span>
                  <span className="text-white">{new Date(billing.currentPeriodEnd).toLocaleDateString()}</span>
                </div>
              )}
